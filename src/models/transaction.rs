...
impl Transaction {
    ...
    fn sign(&mut self, secret_key: &SecretKey) {
        ...
        self.signature = Secp256k1::new().sign_ecdsa(&Message::from_digest_slice(&message).expect("Error here"), secret_key).to_string();        
    }   
    ...
    fn verify(&self, public_key: &PublicKey) -> bool {
        ...
        Secp256k1::new().verify_ecdsa(&Message::from_digest_slice(&message).unwrap(), &signature, public_key).is_ok()
    }
