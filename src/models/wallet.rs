...
use std::{fs::OpenOptions,io::{self, Read},collections::HashMap};
...
impl Wallet {
    ...
    pub fn new(name: String, secret:String) -> Self {
        ...
        let sec_key = SecretKey::from_slice(&Sha256::digest(secret.as_bytes())).expect("32 bytes, within curve order");
        ...
        wallet.save_to_file().expect("Failed to save wallet to file");
    ...
    pub fn update_balance(&mut self, amount: f32) {
        ...
        self.save_to_file().expect("Failed to update wallet in file"); // Update the file
    }

    pub fn get_wallets() -> Option<HashMap<String, Wallet>> {
        let file_path = "wallets.json";

        let wallets: HashMap<String, Wallet> = if let Ok(mut file) = OpenOptions::new().read(true).open(file_path) {
            let mut contents = String::new();
            file.read_to_string(&mut contents).expect("Failed to read wallet file");
            ...
            HashMap::from([
                ("Alice".to_owned(), wallet1),
                ("Bob".to_owned(), wallet2),
            ])
        };
    ...
    fn save_to_file(&self) -> io::Result<()> {
        ...
        let mut wallets: HashMap<String, Wallet> = if let Ok(mut file) = OpenOptions::new()
            .read(true)
            .open(file_path) 
        {
            let mut contents = String::new();
...
impl std::fmt::Display for Wallet {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Wallet: {} - Balance: {} - Secret Key: {:?} - Public Key: {:?}", self.name, self.balance, self.sec_key(), self.pub_key())
    }
}
