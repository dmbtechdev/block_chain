...
use chrono::Utc;
use std::{thread,time::Duration,io::{self, Write},sync::{Arc, Mutex}};

fn main() {

    // Clear the terminal and write the current time
    std::process::Command::new("clear").status().unwrap();
    print!("{}-->", Utc::now().format("Time: %H:%M:%S").to_string().yellow().bold().underline());
    ...
    let _dot_thread = thread::spawn(|| {
        loop {
            ...
            print!("\r{}-->", Utc::now().format("Time: %H:%M:%S").to_string().yellow().bold().underline());
    ...
    for simulate in 1..4{
        ...
        for index in 1..5 { 
            ...
            if transaction.check_balance(&wallet1) {                
                ...
                let message = format!("{} --> {} --> Amount: {} --> Balance: {}",sender,recipient,amount,balance);
        ...
        if transaction_pool.len() > 0 { 
            ...
            let nodes = 10;
            let threads_pool: Vec<_> = (0..nodes).map(|i| {
            ...
            loop {
                ...
                if mining_data.lock().unwrap().mined && mining_data.lock().unwrap().proof_of_work != 0 {
                    ...
                    if mined_block.verify() { 
                        ...
                        println!("Wallet 1 - Remaining balance: {}", wallet1.balance().to_string().red());
                        println!("Wallet 2 - Remaining balance: {}", wallet2.balance().to_string().green());
                        blockchain.write_to_file().expect("Failed to save blockchain to file");
                    }
                    break;
                }
            }
